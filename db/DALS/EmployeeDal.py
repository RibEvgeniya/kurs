...
from db.DALS import SpecialisationDal
from db.models import PermissionsEnum, Employee, Specialisation
...
class EmployeeDAL:
    ...
    async def create_employee(
        ...
        res = await self.session.execute(select(Specialisation).where(Specialisation.name == post_name))
    ...
    async def get_employee_by_username(self, employee_username: str) -> Employee | None:
        query = (
            ...
            .options(joinedload(Employee.specialisation))
        )
